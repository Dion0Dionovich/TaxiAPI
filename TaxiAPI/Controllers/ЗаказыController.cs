        ...
        public async Task<ActionResult<IEnumerable<ЗаказDto>>> GetЗаказы()
        {
            var заказы = await _context.Заказы
                ...
                .Select(z => new ЗаказDto
                {
                    ...
                    Пользователь = new ПользовательDto
                    {
            ...
            try
            {
                ...
                if (request.услуги_id != null && request.услуги_id.Any())
                {
                    var услуги = await _context.Услуги
                        .Where(u => request.услуги_id.Contains(u.услуга_id))
                        .ToListAsync();
                    итоговаяЦена += услуги.Sum(u => u.цена);
                }
                ...
                if (request.скидки_id != null && request.скидки_id.Any())
                {
                    var скидки = await _context.Скидки
                        .Where(s => request.скидки_id.Contains(s.скидка_id) && s.активна)
                        .ToListAsync();

                    foreach (var скидка in скидки)
                    {
                ...
                if (request.скидки_id != null && request.скидки_id.Any())
                {
                    ...
                    foreach (var скидка in скидки)
                    {
                        var суммаСкидки = (заказ.итоговая_цена + суммаСкидок) * скидка.процент_скидки / 100;
